...
export default function SearchBar({
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      setLoading(false);
      // Scroll a la sección de resultados — fix UX abr-2026n: sin esto el
      // usuario no veía los resultados (quedan abajo del fold) y asumía
      // que los TopDeals ARRIBA del SearchBar eran los matches.
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div className="md:col-span-2 relative">
            ...
            <input
              ...
              className="w-full rounded-lg bg-slate-800 border border-slate-600 px-3 py-2.5 min-h-[44px] text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-400"
              autoComplete="off"
              role="combobox"
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg bg-slate-800 border border-slate-600 px-3 py-2.5 min-h-[44px] text-white focus:outline-none focus:ring-2 focus:ring-amber-400"
            />
          </div>
          <div>
            <label
        ...
        <div className="mt-3 grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div className="md:col-span-2 flex items-end">
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-lg bg-amber-400 hover:bg-amber-300 disabled:bg-slate-600 text-slate-900 font-semibold px-4 py-3 min-h-[44px] transition focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-950"
            >
      ...
      {searched && (
        ...
        >
          {error ? (
            <div
              role="alert"
              className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-4 space-y-2"
            >
              <p className="text-amber-300 font-medium">{error}</p>
              <p className="text-sm text-slate-400">
                ¿Quieres explorar lo que hay disponible ahora?{" "}
                <a
                  href="/deals"
                  className="text-amber-300 hover:text-amber-200 underline focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 rounded"
                >
                  Ver todos los chollos activos →
                </a>
              </p>
            </div>
          ) : results.length > 0 ? (
            <>
              <p className="text-slate-300 mb-3 text-sm">
                {results.length}{" "}
                {results.length === 1 ? "oferta encontrada" : "ofertas encontradas"}{" "}
                {origin && (
                  <span>
                    desde <strong className="font-mono text-amber-300">{origin}</strong>
                  </span>
                )}
                {destination && (
                  <span>
                    {" "}hacia <strong className="font-mono text-amber-300">{destination}</strong>
                  </span>
                )}
                .
              </p>
