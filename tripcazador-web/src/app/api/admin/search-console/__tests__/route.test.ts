/**
 * /api/admin/search-console GET route.test.ts — SSS272 (17 may 2026)
 */
...
beforeEach(() => {
  delete process.env.GOOGLE_SEARCH_CONSOLE_KEY;
...
describe("/api/admin/search-console GET — auth", () => {
  it("401 sin sesión", async () => {
    const GET = await importGet();
    const res = await GET();
    expect(res.status).toBe(401);
  });
  it("200 con sesión válida (puede ser not_configured)", async () => {
